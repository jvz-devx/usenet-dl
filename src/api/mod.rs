...
<nzb xmlns="http://www.newzbin.com/DTD/2003/nzb">
  <file subject="test2">
    ...
    async fn test_post_servers_test_endpoint() {
        ...
        assert!(result.latency.is_some(), "Should measure latency even on failure");
        assert!(result.capabilities.is_none(), "Should not have capabilities on failure");
    ...
    async fn test_get_servers_test_endpoint() {
        ...
        let body = axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let results: Vec<(String, crate::types::ServerTestResult)> = serde_json::from_slice(&body).unwrap();
        ...
        // Verify server names
        assert_eq!(results[0].0, "news1.example.com", "First server should be news1.example.com");
        assert_eq!(results[1].0, "news2.example.com", "Second server should be news2.example.com");
        ...
        for (host, result) in &results {
            ...
            assert!(result.error.is_some(), "Server {} should have error message", host);
            assert!(result.latency.is_some(), "Server {} should measure latency", host);
        }
